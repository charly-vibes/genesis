...
fn forbidden_api_patterns() {
    // (pattern, why it's forbidden — shown in the failure message)
    let forbidden: &[(&str, &str)] = &[
        (
            ".with_data(",
            "Output::with_data never existed; data goes in Output::success(data)",
        ),
        (
            ".add_error(",
            "ErrorSink::add_error never existed; use ErrorSink::handle",
        ),
        (
            ".emit_output(",
            "ErrorSink::emit_output never existed; use ErrorSink::handle",
        ),
        (
            "ErrorSink::new()",
            "ErrorSink::new takes a tool_name argument",
        ),
        (
            "emit(format, ",
            "Output::emit requires cli_version and verbosity args",
        ),
        (
            "genesis-vibes = \"0.4\"",
            "stale version pin; Cargo.toml has moved on",
        ),
        (
            "tag = \"v0.4.0\"",
            "stale git tag pin; use the current release tag",
        ),
        (
            "calls `std::process::exit()` internally",
            "maybe_print_version_json returns bool; the caller exits",
        ),
    ];
