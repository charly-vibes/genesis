...
- **dont** — grounded claims and assertions (see `.dont/AGENTS.md`)
- **pretender** — code quality checks (see `pretender.toml`)
- **testaruda** — test selection (see `testaruda.toml`)
- **espectacular** — architecture drift detection (see `.espectacular/AGENTS.md`)
...
Run `pretender check` to evaluate code quality before committing.

- `pretender check` — fast pass/fail scan
- `pretender complexity` — cyclomatic complexity per function
- `pretender duplication` — structural clone detection
- `pretender mutation` — mutation testing wrapper
- `pretender report` — markdown/HTML report
- `pretender doctor` — diagnose configuration

## testaruda

Run `testaruda select` to find affected tests from a code change.

- `testaruda select <diff>` — select affected tests
- `testaruda calibrate` — evaluate prediction ranking
- `testaruda ingest` — ingest test run results
- `testaruda explain <test>` — explain selection rationale
