...
(`passed | failed | rate_limited | invalid_output`), checks with
`ERR_*` codes and reasons, bounds that were applied. Required for
tier-2 rows, optional otherwise: model id (raw, verbatim, `:free`
suffix included) and repetition index. Optional: notes, fixture root
reference. The three vocabularies stay distinct: status (trial
outcome), fault (agent-vs-tool attribution), `ERR_*` code (shared
taxonomy classification of an agent fault).
...
- **No silent substitution**: on rate limiting, a trial ends
  `rate_limited` after one spaced retry; model fallback is a caller-level
  decision that lands in the report. Averaging/dedup belongs to the
  dashboard, not the battery.
